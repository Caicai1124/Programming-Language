...
-- LOWER BOUND
-- Find the first greater or equal number, then return previous one
...
main = do 
  ...
  -- NO WS in the head
  if (head inputStr == ' ' || head inputStr == '\t') then do
    printf "ERR\n"
    exitWith (ExitFailure 2)
  else do

  -- NO WS in the end
  if (last inputStr == ' ' || last inputStr == '\t') then do
    printf "ERR\n"
  ...
  -- Deal with "BOUNDS"
    ...
    -- Validation Check
    let part = (partition( `elem` ['.']) number)

    -- At most one "." is allowed
    if (length( fst part)) > 1 then do
      printf "ERR\n"
      exitWith (ExitFailure 8)
    else do
       
    -- All char should be digit apart from '.'
    if (all isDigit (snd part)) == False then do
      printf "ERR\n"
