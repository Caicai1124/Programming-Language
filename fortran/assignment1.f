...
c           Compose "-" and "." to check precision lose
            ...
            read(cur_num_buf, '(F40.6)',ERR=888) cur_num
...
c         DEAL WITH ROUNDING OFF
            read(buf(stt_b:stt_b+p_b-1),'(F40.6)',ERR=777) tmp_num
            read(buf_in(stt_in:stt_in+p_in-1),'(F40.6)',ERR=777) tmp
...
c         DEAL WITH ROUNDING OFF
            read(buf(p_b+1:fin_in),'(F40.6)',ERR=777) tmp_num
            read(buf_in(p_in+1:fin_b),'(F40.6)',ERR=777) tmp
...
c         No negative sign in outfloat

          if ( sum .lt. -0.005 ) goto 888
          if ( avg .lt. -0.005 ) goto 888
          if ( cur_min .lt. -0.005 ) goto 888
          if ( cur_max .lt. -0.005 ) goto 888

c         Get rid of -0.00
          if ( sum .gt. -0.005 .and. sum .lt. 0.005) sum = 0
          if ( avg .gt. -0.005 .and. avg .lt. 0.005) avg = 0
          if ( cur_min.gt.-0.005.and.cur_min.lt.0.005)cur_min= 0
          if ( cur_max.gt.-0.005.and.cur_max.lt.0.005)cur_max=0

          write(*,'(A,F30.2)') 'Sum: ', sum 
          write(*,'(A,F30.2)') 'Average: ', avg
          write(*,'(A,F30.2)') 'Minimum: ', cur_min 
          write(*,'(A,F30.2)') 'Maximum: ', cur_max
