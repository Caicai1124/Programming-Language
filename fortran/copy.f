...
c       READ IN ALL ELEMENTS
          ...
          character kbd_in*256
          character cur_max*256, cur_min*256
          character get_max*256, get_min*256
          character int_add*256, frac_add*256
          character pos_int, pos_frc, neg_int, neg_frc
...
c           Check Input is positive or negative
            ...
            if ( k .eq. 1 ) then
              ...
              cur_max = get_max(cur_max, sign, kbd_in)
              cur_min = get_min(cur_min, sign, kbd_in)
            endif
            if(INDEX(kbd_in(stt:fin), '.') .ne. 0) then
              pnt = INDEX(kbd_in(stt:fin), '.')
c             NO LEADING
              if (pnt .eq. stt) then
...
c             With Leading, but optional is empty
                ...
                if(is_invalid_int(kbd_in(stt: fin-1)) .eq. 1) then
                  goto 888
                endif
...
c             Contain both integer part and fraction part
                ...
                if(is_invalid_int(kbd_in(pnt+1: fin)) .eq. 1) then
                  goto 888
                endif
                if (sign .eq. 1) then
  ...
  888     call print_error
        ...
        character*(*)function get_max(s1,sign,s2)
          character*(*) s1, s2
          integer sign
c          get_max = s1
          return
